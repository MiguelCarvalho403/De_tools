...
def division_separate(partially_separated_pde, funcs, sep_variables):
    from sage.symbolic.operators import add_vararg
    equation = partially_separated_pde
    mix = equation[0].expand()
    if mix.operator() is add_vararg:
        mix = mix.operands()
    else:
        mix = [mix]
    co_diffx = 1
    ...
    if partially_separated_pde[1] == 0:
        for term in mix:
            if diff(funcs[0], sep_variables[0]) in term.operands() or diff(funcs[0], sep_variables[0], 2) in term.operands():
                co_diffx = prod([k for k in term.operands() if k != diff(funcs[0], sep_variables[0]) and k != diff(funcs[0], sep_variables[0], 2) and sep_variables[0] not in k.variables()])
                break
                
    if partially_separated_pde[2] == 0:    
        for term in mix:
            if diff(funcs[1], sep_variables[1]) in term.operands() or diff(funcs[1], sep_variables[1], 2) in term.operands():
                co_diffy = prod([k for k in term.operands() if k != diff(funcs[1], sep_variables[1]) and k != diff(funcs[1], sep_variables[1], 2) and sep_variables[1] not in k.variables()])
                break
            
    d = (co_diffx*co_diffy)

    for index, eq in enumerate(partially_separated_pde):
        partially_separated_pde[index] = (eq/d).expand()
    
    print('division terminate')
    
    return d, partially_separated_pde
        ...
        if is_sum_separable(partially_separated_pde[0], sep_variables): # Determine whether the PDE can be separated into a sum of single-variable terms
            ...
            if not common_factor==1:
                ...
                if not common_factor==1:
                    ...
                    divisor, partially_separated_pde = division_separate(partially_separated_pde, funcs, sep_variables)
                    if not divisor == 1:
                        return _separate_pde(sum(partially_separated_pde), funcs, sep_variables, sep_constant, check=check, show_log=show_log, show_eqs=show_eqs, initial_pde=initial_pde/divisor)
                    else:
                        return partially_separated_pde


def separate_pde(pde, funcs, sep_variables, sep_constant, is_list = False, factor=1, check=True, show_log=True, show_eqs=True, initial_pde=None):

    #if not pde.has(funcs[0]):
    #    pde.substitute_function(funcs[2] == funcs[0]*funcs[1])
        
    #pde = (pde/(funcs[0]*funcs[1])).expand()
    
    separated_equations = _separate_pde(pde, funcs, sep_variables, sep_constant, is_list, factor, check, show_log, show_eqs, initial_pde)
    return separated_equations
